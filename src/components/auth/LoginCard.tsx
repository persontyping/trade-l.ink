...
export default function LoginCard({
  ...
  async function handleMagicLink() {
    ...
    formData.set("email", modal.email);
    formData.set("action", "magicLink");
  ...
  async function handleUpdateDetails() {
    if (!modal) return;
    window.location.href = `/account/update?email=${encodeURIComponent(modal.email)}`;
  }
      ...
      {modal && (
        <div className="flex-1 h-full border border-pink-600 flex rounded-lg z-50 mt-30">
          <div className="p-6 rounded-lg w-full ">
            <p className="mb-4 text-center">Login failed. What would you like to do?</p>
            <div className="flex justify-center gap-4">
              <button
                onClick={handleMagicLink}
                className="btn-b btn-b1:focus btn-b1:hover px-4 py-2 rounded-lg "
              >
                Send Magic Link
              </button>
              <button
                onClick={handleUpdateDetails}
                className="btn1-b btn-b:focus btn-b:hover px-4 py-2 border rounded-lg"
              >
                Update Details
              </button>
            </div>
